...
localDebugger.on('connection', (socket) => {
    ...
    socket.on('data', (data) => {
        ...
        skillInvoker[lambdaHandlerName](body, null, (_invokeErr, response) => {
            ...
            const socketOutput  = `HTTP/1.1 200 OK${HTTP_HEADER_DELIMITER}Content-Type: application/json;charset=UTF-8${HTTP_HEADER_DELIMITER}Content-Length: ${Buffer.byteLength(response, 'utf8')}${HTTP_BODY_DELIMITER}${response}`;
